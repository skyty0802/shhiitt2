...
// 관리자 비밀번호 로직 (191113으로 변경됨)
document.getElementById('adminTrigger').addEventListener('click', () => {
    const password = prompt("관리자 비밀번호를 입력하세요.");
    if (password === "191113") {
        alert("비밀번호가 일치합니다. 관리자 모드를 활성화합니다.");
    ...
    list.innerHTML = students.map((s, i) => `
        <li class="${s.pref}-pref-item">
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <button onclick="students.splice(${i}, 1); renderStudents();" style="background:#f02849; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;">삭제</button>
            </div>
        </li>
    `).join('');
}
...
function clearStudents() {
    if (confirm("초기화하시겠습니까?")) { students = []; renderStudents(); }
}
...
function generateMap() {
    ...
    for (let i = 0; i < rows * cols; i++) {
        ...
        cell.onclick = () => {
            ...
            if (cell.classList.contains('unavailable')) { cell.innerText = 'X'; unavailableSeats.add(i); }
            else { cell.innerText = ''; unavailableSeats.delete(i); }
        };
        cell.ondblclick = () => {
            if (cell.classList.contains('unavailable')) return;
            const name = prompt("고정할 학생 이름:");
            if (name) { 
                fixedSeats[i] = name; 
...
function renderConstraints() {
    document.getElementById('constraintList').innerHTML = constraints.map((c, i) => `
        ...
        <button onclick="constraints.splice(${i},1); renderConstraints();" style="background:#f02849; color:white; border:none; padding:5px 10px; border-radius:4px; cursor:pointer;">삭제</button></li>
    `).join('');
}
...
function startShuffle() {
    ...
    else alert("조건에 맞는 배치를 찾지 못했습니다.");
}
...
function checkConstraints(layout, rows, cols) {
    for (let c of constraints) {
        ...
        if (c.type === 'enemy' && i1 !== -1 && i2 !== -1 && isNear) return false;
    }
    return true;
}
